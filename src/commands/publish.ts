...
function gatherOptions(
  ...
  try {
    ...
    options.unlisted && (cfg.unlisted = options.unlisted);
    options.repository &&
      (cfg.repository = urlType(
        { name: "repository", value: options.repository, label: "", type: "" },
      ));
    options.wallet &&
      (cfg.wallet = walletType(
        { name: "wallet", value: options.wallet, label: "", type: "" },
      ));
...
async function publishCommand(options: Options, name?: string) {
  ...
  if (
    ...
    existing?.uploads
        ...
        .indexOf(`${egg.name}@${egg.version}`) !== -1
  ) {
    log.error(
  ...
  const module: PublishModule = {
    ...
    entry: egg.entry,
    wallet: egg.wallet ? await getWallet(egg.wallet) : null,
    upload: true,
    malicious: false,
    apiKey,
  };
